...
const Products: React.FC = () => {
  ...
  const filteredProducts = useMemo(() => {
    const filtered = products.filter(product => {
      ...
      console.log('Product:', product.category, product.colors, 'Filter:', filters.category, filters.colors);
      ...
      const matchesColors =
        ...
        filters.colors.some(
          color => colors.map(c => c.toLowerCase()).includes(color.toLowerCase())
        );
      const matchesPrice = product.price >= filters.priceRange![0] && product.price <= filters.priceRange![1];
      // Fix: Use product.colors for color filtering
